...
coc_funded <- Funder %>%
  dplyr::filter(Funder %in% c(1:7) &
           ...
           (ProjectID %in% c(keepers, retired) |
              (
                lubridate::ymd(StartDate) <= lubridate::ymd(rm_dates$hc$project_eval_end) &
                  (is.na(EndDate) |
                     lubridate::ymd(EndDate) >= lubridate::ymd(rm_dates$hc$project_eval_end))
              ))) %>%
...
pe_adults_entered <-  co_adults_served %>%
  ...
  dplyr::ungroup() %>%
  dplyr::filter(HMIS::entered_between(., rm_dates$hc$project_eval_start, rm_dates$hc$project_eval_end) &
...
summary_pe_coc_scoring <- pe_coc_funded %>%
  ...
  dplyr::mutate(
    ...
    PrioritizationWorkgroupMath = dplyr::case_when(
      lubridate::today() <= lubridate::ymd(rm_dates$hc$project_eval_docs_due) &
      ...
      lubridate::ymd(DateReceivedPPDocs) > lubridate::ymd(rm_dates$hc$project_eval_docs_due) ~
    ...
    HousingFirstDQ = dplyr::case_when(
      ...
      lubridate::ymd(DateReceivedPPDocs) > lubridate::ymd(rm_dates$hc$project_eval_docs_due) ~ 5
    ),
    HousingFirstScore = dplyr::case_when(
      is.na(DateReceivedPPDocs) |
        is.na(HousingFirstScore) ~ -10,
      lubridate::ymd(DateReceivedPPDocs) > lubridate::ymd(rm_dates$hc$project_eval_docs_due) ~ -10,
      lubridate::ymd(DateReceivedPPDocs) <= lubridate::ymd(rm_dates$hc$project_eval_docs_due) ~ HousingFirstScore
    ),
