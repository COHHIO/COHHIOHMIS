...
project_evaluation <- function(
  ...
  pe <- list()
  pe$clients_served <- peval_filter_select(co_clients_served, vars = vars$prep,  served = TRUE)
  # several measures will use this

  # Checking for deceased hohs for points adjustments


  hoh_exits_to_deceased <- pe$clients_served %>%
  ...
  pe$adults_entered <- peval_filter_select(co_adults_served, vars = vars$prep, distinct = FALSE) |>
  ...
  pe$hohs_entered <- peval_filter_select(co_hohs_entered, vars = vars$prep, entered = TRUE, distinct = FALSE)

  # for ncb logic
  # Adults who moved in and exited during date range

  pe$adults_moved_in_leavers <- peval_filter_select(co_adults_moved_in_leavers, vars = vars$prep, stayed = TRUE, exited = TRUE)


  # health insurance
  # Clients who moved in and exited during date range

  pe$clients_moved_in_leavers <- peval_filter_select(co_clients_moved_in_leavers, vars = vars$prep, stayed = TRUE, exited = TRUE)

  # exits to PH, but needs an added filter of only mover-inners
  # Heads of Household who were served during date range

  pe$hohs_served <- peval_filter_select(co_hohs_served, vars = vars$prep, served = TRUE)

  pe$hohs_served_leavers <- peval_filter_select(co_hohs_served, vars = vars$prep, served = TRUE, exited = TRUE)

  # own housing and LoS
  # Heads of Household who moved in and exited during date range

  pe$hohs_moved_in_leavers <- peval_filter_select(co_hohs_moved_in_leavers, vars = vars$prep, stayed = TRUE, exited = TRUE)
  ...
  #       is.na(Destination) | ymd(ExitAdjust) > ymd(rm_dates$hc$project_eval_end) ~
  #         "Still in Program at Report End Date",
  ...
  #     PersonalID = as.character(PersonalID)
  #   ) %>%
  #   select(all_of(vars$we_want), OwnHousingDQ, Destination, DestinationGroup)
  #
...
Project_Evaluation_legacy <- function(
             clarity_api,
             app_env,
             e = rlang::caller_env()
            ) {
...
if (missing(app_env))
  ...
  # rm_dates$hc$project_eval_start <- mdy("01012019") # for comparison purposes
  # rm_dates$hc$project_eval_end <- mdy("12312019")
...
coc_funded <- Funder %>%
  ...
  dplyr::left_join(Project[c("ProjectID",
                      ...
                      "ProjectRegion")], by = "ProjectID") %>%
  dplyr::filter(HMISParticipatingProject == 1 &
           (ProjectRegion != 0 | is.na(ProjectRegion))) %>%
  dplyr::select(ProjectType,
         ProjectName,
         ProjectID)
...
pe_adults_moved_in_leavers <-  co_adults_moved_in_leavers %>%
  HMIS::stayed_between(rm_dates$hc$project_eval_start, rm_dates$hc$project_eval_end) |>
  HMIS::exited_between(rm_dates$hc$project_eval_start, rm_dates$hc$project_eval_end) |>
  dplyr::select("PersonalID", "ProjectID", "EnrollmentID") %>%
  dplyr::inner_join(pe_coc_funded, by = "ProjectID") %>%
  dplyr::left_join(Client, by = "PersonalID") %>%
...
#   filter(stayed_between(., rm_dates$hc$project_eval_start, rm_dates$hc$project_eval_end)) %>%
...
pe_hohs_moved_in_leavers <-  co_hohs_moved_in_leavers %>%
  dplyr::filter(HMIS::stayed_between(., rm_dates$hc$project_eval_start, rm_dates$hc$project_eval_end) &
           HMIS::exited_between(., rm_dates$hc$project_eval_start, rm_dates$hc$project_eval_end)) %>%
  dplyr::select("PersonalID", "ProjectID", "EnrollmentID") %>%
  dplyr::inner_join(pe_coc_funded, by = "ProjectID") %>%
  dplyr::left_join(Client, by = "PersonalID") %>%
