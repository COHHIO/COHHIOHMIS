...
qpr_spdats_county <-
  ...
  dplyr::filter(
    ProjectType %in% c(1, 2, 4, 8) &
      RelationshipToHoH == 1 &
      lubridate::ymd(ScoreDate) <= lubridate::ymd(EntryDate) &
      !CountyServed %in% c(
        "Montgomery",
        "Cuyahoga",
        "Mahoning",
  ...
  dplyr::slice_max(lubridate::ymd(EntryDate)) %>% # most recent EE
  dplyr::slice_max(lubridate::ymd(ScoreDate)) %>% # most recent score
...
qpr_spdats_project <- dplyr::left_join(Entries, Scores, by = "PersonalID") %>%
  dplyr::select(-ProjectType,
         -OperatingStartDate,
         -OperatingEndDate) %>%
  dplyr::filter(
    RelationshipToHoH == 1 &
      (lubridate::ymd(ScoreDate) <= lubridate::ymd(EntryDate) | is.na(ScoreDate)) &
      !CountyServed %in% c(
        "Montgomery",
        "Cuyahoga",
        "Mahoning",
  ...
  dplyr::slice_max(lubridate::ymd(ScoreDateAdjusted)) %>%
