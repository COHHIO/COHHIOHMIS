...
spms <- function(spm_xlsx_file = clarity.looker::hud_filename("hudx-223-ad", dirs$spm)) {
  ...
  spm <- load_spm(spm_xlsx_file)
}

SPMs_legacy <- function(
  clarity_api,
  app_env,
  e = rlang::caller_env()
) {
  ...
  rename_file <-
    function(CoC = "OH-507",
             ...
             pattern = "(0700 -)") {
      directory <- dplyr::case_when(CoC == "OH-507" ~ "SPM_data_BoS",
  ...
  check_loth_a <- function(CoC = "OH-507", subdirectory = "Current") {
    ...
    loth_a <-
      readxl::read_xls(
        ...
        sheet = 3,
        range = readxl::cell_cols("B2:C9")
      ) %>%
      dplyr::select("Prompt" = 2, "Selection" = 3) %>%
      utils::head(n = 7L) %>%
      tidyr::spread(Prompt, Selection) %>%
      dplyr::select(
        "EDA" = 1,
        "EffectiveDate" = 2,
        "ReportEnd" = 3,
    ...
    rpt_0700a <- loth_a %>%
      dplyr::select(ReportStart, ReportEnd, PriorYear, EffectiveDate) %>%
      dplyr::mutate(CoCName = CoC,
                    CurrentOrPrior = subdirectory,
                    ReportName = "0700a")

    assign(paste0(stringr::str_remove(CoC, "-"), "_", subdirectory, "_rpt_0700a"),
           rpt_0700a,
           envir = .GlobalEnv)

    loth_ees <- readxl::read_xls(
      paste0(directory,
             "/",
             subdirectory,
             "/0700a.xls"),
      sheet = 1,
      range = readxl::cell_cols("B2:E4")
    ) %>%
      dplyr::select("Metric1a" = 1, "ClientCount" = 2, "AvgLoT" = 3, "MedLoT" = 4) %>%
      dplyr::mutate(CoCName = CoC,
                    CurrentPrior = subdirectory)

    assign(paste0("spm_Metric_1a_", stringr::str_remove(CoC, "-"), "_", subdirectory),
           loth_ees,
           envir = .GlobalEnv)
    ...
    if (nrow(
      ...
      dplyr::filter(
        ...
        stringr::str_sub(CoCInfo, 1, 6) != CoC |
        ReportEnd - lubridate::years(1) != ReportStart |
        ReportStart - lubridate::years(1) != PriorYear
      )
    ) > 0)
  ...
  check_loth_b <- function(CoC = "OH-507", subdirectory = "Current"){
    ...
    loth_b <- readxl::read_xls(paste0(directory,
                                      "/",
                                      subdirectory,
                                      "/0700b.xls"),
                               sheet = 3,
      ...
      dplyr::select("EDA" = 1,
                    ...
                    "CoCInfo" = 6,
                    "Providers" = 7) %>%
      dplyr::mutate(ReportStart = as.integer(ReportStart),
                    ReportEnd = as.integer(ReportEnd),
                    PriorYear = as.integer(PriorYear),
                    EffectiveDate = as.integer(EffectiveDate),
                    ReportStart = as.Date(ReportStart, origin = "1899-12-30"),
    ...
    assign(paste0(stringr::str_remove(CoC, "-"), "_", subdirectory, "_rpt_0700b"),
           rpt_0700b,
           envir = .GlobalEnv)

    loth_self_report <- readxl::read_xls(
      paste0(directory,
             "/",
             subdirectory,
             "/0700b.xls"),
    ...
    assign(paste0("spm_Metric_1b_", stringr::str_remove(CoC, "-"), "_", subdirectory),
           loth_self_report,
           envir = .GlobalEnv)
  ...
  check_recurrence <-
    function(CoC = "OH-507", subdirectory = "Current") {
      ...
      worksheet_count <- length(readxl::excel_sheets(paste0(
        directory,
        "/",
        subdirectory,
        "/0701.xls"
      ...
      rpt_0701 <- recurrence %>%
        dplyr::select(PriorYear, Prior2Year, ReportEnd, EffectiveDate) %>%
        dplyr::mutate(CoCName = CoC,
                      CurrentOrPrior = subdirectory,
                      ReportName = "0701")

      assign(paste0(stringr::str_remove(CoC, "-"), "_", subdirectory, "_rpt_0701"),
             rpt_0701,
             envir = .GlobalEnv)

      recurrence_data <- readxl::read_xls(
        paste0(directory,
               "/",
               subdirectory,
               "/0701.xls"),
        sheet = what_sheet_is_701_data_on,
        range = readxl::cell_rows(3:10)
      ) %>%
        dplyr::select(
          "ProjectType" = 1,
      ...
      assign(paste0("spm_Metric_2_", stringr::str_remove(CoC, "-"), "_", subdirectory),
             recurrence_data,
             envir = .GlobalEnv)
      ...
      if (nrow(
        ...
        dplyr::filter(
          ...
          EffectiveDate != ReportEnd |
          stringr::str_sub(CoCInfo, 1, 6) != CoC |
          ReportEnd - lubridate::years(2) != PriorYear |
          ReportEnd - lubridate::years(3) != Prior2Year
        )
      ) > 0)
  ...
  check_homeless_count <-
    function(CoC = "OH-507", subdirectory = "Current") {
      ...
      homeless_count <- readxl::read_xls(
        paste0(directory,
               "/",
               subdirectory,
               "/0702.xls"),
      ...
      assign(paste0(stringr::str_remove(CoC, "-"), "_", subdirectory, "_rpt_0702"),
             rpt_0702,
             envir = .GlobalEnv)

      homeless_count_data <- readxl::read_xls(
        paste0(directory,
               "/",
               subdirectory,
               "/0702.xls"),
        sheet = 1,
        range = readxl::cell_cols("B2:D6")
      ) %>%
        dplyr::select("Type" = 1, "Count" = 3) %>%
        dplyr::mutate(CoCName = CoC,
                      CurrentPrior = subdirectory)

      assign(paste0("spm_Metric_3_", stringr::str_remove(CoC, "-"), "_", subdirectory),
             homeless_count_data,
             envir = .GlobalEnv)
  ...
  check_income <- function(CoC = "OH-507", subdirectory = "Current") {
    ...
    income <- readxl::read_xls(
      paste0(directory,
             "/",
             subdirectory,
             "/0703.xls"),
      sheet = worksheet_count,
      range = readxl::cell_cols("B2:e9")
    ) %>%
      dplyr::select("Prompt" = 2, "Selection" = 5) %>%
      utils::head(n = 7L) %>%
      tidyr::spread(Prompt, Selection) %>%
      dplyr::select(
        "EDA" = 1,
    ...
    assign(paste0(stringr::str_remove(CoC, "-"), "_", subdirectory, "_rpt_0703"),
           rpt_0703,
           envir = .GlobalEnv)

    income_data <- readxl::read_xls(paste0(directory,
                                           "/",
                                           subdirectory,
                                           "/0703.xls"),
                                    sheet = 1) %>%
      dplyr::select("ClientsCounted" = 1, "Counts" = 3) %>%
      dplyr::filter(!is.na(ClientsCounted))
    ...
    assign(paste0("spm_Metric_4_", stringr::str_remove(CoC, "-"), "_", subdirectory),
           income_data,
           envir = .GlobalEnv)
  ...
  check_first_timers <-
    function(CoC = "OH-507", subdirectory = "Current") {
      ...
      first_timers <- readxl::read_xls(paste0(directory,
                                              "/",
                                              subdirectory,
                                              "/0704.xls"),
                                       sheet = 4,
      ...
      assign(paste0(stringr::str_remove(CoC, "-"), "_", subdirectory, "_rpt_0704"),
             rpt_0704,
             envir = .GlobalEnv)

      first_timers_data <- readxl::read_xls(paste0(directory,
                                                   "/",
                                                   subdirectory,
                                                   "/0704.xls"),
                                            sheet = 1) %>%
      ...
      assign(paste0("spm_Metric_5_", stringr::str_remove(CoC, "-"), "_", subdirectory),
             first_timers_data,
             envir = .GlobalEnv)
  ...
  check_exits_to_ph <-
    function(CoC = "OH-507", subdirectory = "Current") {
      ...
      assign(paste0(stringr::str_remove(CoC, "-"), "_", subdirectory, "_rpt_0706"),
             rpt_0706,
             envir = .GlobalEnv)

      exits_to_ph_data <- readxl::read_xls(paste0(directory,
                                                  "/",
                                                  subdirectory,
                                                  "/0706.xls"),
                                           sheet = 1) %>%
      ...
      exits_out_to_ph_current <- exits_to_ph_data[c(1:4), ] %>%
        dplyr::mutate(Metric = "7a")

      exits_lh_current <- exits_to_ph_data[c(6:8), ] %>%
        dplyr::mutate(Metric = "7b1")

      exits_ph_current <- exits_to_ph_data[c(10:12), ] %>%
        dplyr::mutate(Metric = "7b2")

      exits_to_ph_data <- rbind(exits_out_to_ph_current,
                                exits_lh_current,
                                exits_ph_current)

      assign(paste0("spm_Metric_7_", stringr::str_remove(CoC, "-"), "_", subdirectory),
             exits_to_ph_data,
             envir = .GlobalEnv)
  ...
  current_rpts <- OH507_Current_rpt_0700a %>%
    dplyr::full_join(OH507_Current_rpt_0700b) %>%
    dplyr::full_join(OH507_Current_rpt_0701) %>%
    dplyr::full_join(OH507_Current_rpt_0702) %>%
    dplyr::full_join(OH507_Current_rpt_0703) %>%
    dplyr::full_join(OH507_Current_rpt_0704) %>%
    dplyr::full_join(OH504_Current_rpt_0700a) %>%
    dplyr::full_join(OH504_Current_rpt_0700b) %>%
    dplyr::full_join(OH504_Current_rpt_0701) %>%
    dplyr::full_join(OH504_Current_rpt_0702) %>%
    dplyr::full_join(OH504_Current_rpt_0703) %>%
    dplyr::full_join(OH504_Current_rpt_0704)

  prior_rpts <- OH507_Prior_rpt_0700a %>%
    dplyr::full_join(OH507_Prior_rpt_0700b) %>%
    dplyr::full_join(OH507_Prior_rpt_0701) %>%
    dplyr::full_join(OH507_Prior_rpt_0702) %>%
    dplyr::full_join(OH507_Prior_rpt_0703) %>%
    dplyr::full_join(OH507_Prior_rpt_0704) %>%
    dplyr::full_join(OH504_Prior_rpt_0700a) %>%
    dplyr::full_join(OH504_Prior_rpt_0700b) %>%
    dplyr::full_join(OH504_Prior_rpt_0701) %>%
    dplyr::full_join(OH504_Prior_rpt_0702) %>%
    dplyr::full_join(OH504_Prior_rpt_0703) %>%
    dplyr::full_join(OH504_Prior_rpt_0704)

  mahoning_current <- current_rpts %>%
    dplyr::filter(CoCName == "OH-504") %>%
    dplyr::mutate(CheckStart = dplyr::if_else(min(ReportStart, na.rm = TRUE) ==
                                                max(ReportStart, na.rm = TRUE),
                                              0, 1),
                  CheckEnd = dplyr::if_else(min(ReportEnd, na.rm = TRUE) ==
                                              max(ReportEnd, na.rm = TRUE),
                                            0, 1),
                  CheckPrior = dplyr::if_else(min(PriorYear, na.rm = TRUE) ==
                                                max(PriorYear, na.rm = TRUE),
                                              0, 1),
                  CheckPrior2 = dplyr::if_else(is.na(Prior2Year) |
                                                 Prior2Year + lubridate::years(1) == PriorYear,
                                               0, 1))

  bos_current <- current_rpts %>%
    dplyr::filter(CoCName == "OH-507") %>%
    dplyr::mutate(CheckStart = dplyr::if_else(min(ReportStart, na.rm = TRUE) ==
                                                max(ReportStart, na.rm = TRUE),
                                              0, 1),
                  CheckEnd = dplyr::if_else(min(ReportEnd, na.rm = TRUE) ==
  ...
  spm_Metric_1a <- rbind(
    ...
    tidyr::pivot_wider(id_cols = c(Metric1a, CoCName, CurrentPrior),
  ...
  spm_Metric_1b <- rbind(
    ...
    tidyr::pivot_wider(id_cols = c(Metric1b, CoCName, CurrentPrior),
                       names_from = CurrentPrior,
                       values_from = c(ClientCount, AvgLoT, MedLoT))
  ...
  spm_Metric_2 <- rbind(
    ...
    tidyr::pivot_wider(id_cols = c(ProjectType, CoCName, CurrentPrior),
                       names_from = CurrentPrior,
  ...
  spm_Metric_3 <- rbind(
    ...
    tidyr::pivot_wider(id_cols = c(Type, CoCName, CurrentPrior),
                       names_from = CurrentPrior,
                       values_from = Count)
  ...
  spm_Metric_4 <- rbind(
    ...
    tidyr::pivot_wider(id_cols = c(ClientsCounted, CoCName, CurrentPrior, Metric),
                       names_from = CurrentPrior,
                       values_from = Counts)
  ...
  spm_current_end_date <- min(bos_current$ReportEnd, na.rm = TRUE) - lubridate::days(1)

  spm_prior_start_date <- min(bos_prior$ReportStart, na.rm = TRUE)

  spm_prior_end_date <- min(bos_prior$ReportEnd, na.rm = TRUE) - lubridate::days(1)
  ...
  rm(list = lubridate::setdiff(ls(), ls(pattern = "spm_")))
